    ...
    public function request($id = '', $extra = '') {
        ...
        if ($result && $result != " ") {
            ...
            if (count($xml_errors) > 0) {
                $errors_string = array_map(function($error) {
                    return $error->line . " : " .  $error->message;
                }, $xml_errors);
                $this->_error = implode('\n', $errors_string);
                libxml_clear_errors();
                throw new InvoiceXpressRequestException("Empty or invalid XML response : " . $this->_error 
